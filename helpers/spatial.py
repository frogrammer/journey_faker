...
import random
import haversine
from helpers import azmaps, config
...
def get_random_latlng():
    ...
    verified_position = asyncio.run(azmaps.nearest_poi(*random_latlng))['results'][0]['position']
...
def get_random_latlng_batch(batch_size:float=1):
    ...
    verified_pois =loop.run_until_complete(asyncio.gather(*[azmaps.nearest_poi(*ll) for ll in random_latlngs]))
